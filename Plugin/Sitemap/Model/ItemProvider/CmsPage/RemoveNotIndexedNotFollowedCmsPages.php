...
class RemoveNotIndexedNotFollowedCmsPages
{
    ...
    public function afterGetItems(\Magento\Sitemap\Model\ItemProvider\CmsPage $cmsPage, $result)
    {
        ...
        foreach ($result as $pageId => $cmsPageData) {
            if (isset($cmsPagesMetaRobotsAttributes[$pageId]['meta_robots_option']) && $this->metaRobotsTagHelper->isNoIndexOption((int) $cmsPagesMetaRobotsAttributes[$pageId]['meta_robots_option'])) {
                unset($result[$pageId]);
            }
        }
